...
<template>
  ...
  <AuthenticatedLayout>
    <main class="mt-[-20px]">
      ...
      <section class="mt-2 bg-white mb-3" v-if="exist">
        <div class="w-full p-4 flex flex-col gap-2">
          <div class="flex items-center gap-4">
            ...
            <h3 v-if="$page.props.auth.user.id === users.id" class="text-[12px] font-extrabold text-gray-600 py-2 px-1 rounded-full border basis-[85%] bg-gray-100"
              @click="showModalPublication">
              A quoi vous pensez ?
            </h3>
            <h3 v-else class="text-[12px] font-extrabold text-gray-600 py-2 px-1 rounded-full border basis-[85%] bg-gray-100"
              @click="showModalPublication">
        ...
        <div id="divPost" class="hidden z-40 bg-white top-[62px] fixed bottom-0 left-0 right-0 rounded-l-lg rounded-r-lg">
          <div class="relative text-gray-600 text-sm font-bold py-3 border-gray-300 border-b-[1px] mb-2">
            <div class="mx-auto w-[90%] flex items-center justify-between">
              ...
              <button class="text-sky-500 font-bold" v-if="$page.props.auth.user.id === users.id" @click="publishPost()">
                Publier
              </button>
              <button v-else class="text-sky-500 font-bold" @click="publishPost(user.id)">
                Publier
              </button>
            </div>
          </div>
          <div class="mx-auto w-[95%] overflow-y-auto h-[500px]">
            ...
            <button v-if="$page.props.auth.user.id === users.id"
              class="mt-4 w-full bg-[#0c7fb9] text-white text-center p-2 rounded-lg" type="button" @click="publishPost()">
              PUBLIER
            </button>
            <button v-else class="mt-4 w-full bg-[#0c7fb9] text-white text-center p-2 rounded-lg" type="button"
      ...
      <div class="bg-white" v-if="postsUser.length > 0 || allFiles.length > 0">
        <section class="mt-2 bg-white py-1" v-if="postsUser.length > 0">
          <div class="flex flex-col">
            <div v-for="(el, index) in postsUser" :key="index" class="border-b-[1px] pb-2 shadow-sm my-1">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 px-2">
                  <img :src="el.image_user !== null
                      ? `/storage/profilImage/${el.image_user}`
                      : `/storage/images/easy.png`
                    " alt="image_de_profil" class="rounded-full w-[50px] h-[50px]" />
                  <div>
                ...
                <div class="relative basis-[2%] mr-2">
                  ...
                  <transition>
                    ...
                    <div :id="'posts-' + index" v-else-if="users.id === $page.props.auth.user.id"
                      class="allPosts hidden w-[160px] absolute right-0 top-4 border-gray-400 border-[1px] bg-white rounded">
                      <ul class="cursor-pointer">
                        <li class="py-2 px-1.5 hover:bg-gray-200 text-[13px]" @click="deleteImage(el, index)"
                          v-if="users.id === $page.props.auth.user.id">
              ...
              <div class="mt-[10px]">
                <p v-if="el.bgc !== null && el.body.trim().split(/\s+/).length < 40" class="w-[90%] text-sm mx-auto mb-2"
                ...
                <p v-else class="text-sm w-[97%] mx-auto mb-2 px-2 max-h-[500px] overflow-y-auto text-justify"
                  v-html="el.body"></p>
                <img v-if="el.image !== null" :src="`/storage/post_images_videos/${el.image}`" alt="image_de_profil"
        ...
        <section class="mb-2 bg-white py-1" v-if="allFiles.length > 0">
          <div class="flex flex-col">
            <div v-for="(el, index) in allFiles" :key="index" class="border-b-[1px] pb-2 shadow-sm my-1">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2 px-2">
              ...
              <div class="w-[90%] mx-auto mt-[2px] cursor-pointer hover:bg-sky-100 px-1 rounded" v-if="el.likes > 0">
                ...
                <span v-else-if="el.trueVariable && el.likes == 1" class="text-gray-600 text-sm">Vous avez aimé cette
                  photo.</span>
                <span v-else-if="el.likes == 1" class="text-gray-600 text-sm">{{ el.likes }} personne(s) a aimé cette
                  photo.</span>
