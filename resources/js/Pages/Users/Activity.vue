...
<template>
    ...
    <AuthenticatedLayout>
        <main class="mt-[-20px]">
            <ComposantUser :filesProfil="profil" :covers="cover" :lastImage="lImg" :niveau="'activity'"
                :followin="userfollow" :followe="followers" :usersIdentifiant="user" :numberLik="numberLike" />

                <section class="my-4 bg-white py-1">
                    <div class="flex flex-col">
                        <div v-for="(el, index) in allFiles" :key="index" class="border-gray-400 border-b-[1px] pb-2 mb-2" @click="viewPostProfil(getLastImgProfil.user_id, getLastImgProfil.id)">
                            <div class="mt-[20px] flex items-center gap-2 px-2">
                                ...
                                <div>
                                    ...
                                    <p class="italic text-[12px] text-gray-600 font-bold">Publié le {{ el.created_at.split("T")[0] }}</p>
                                </div>
                            </div>
                    ...
                    <div class="mt-2 flex w-[90%] mx-auto justify-between items-center">
                        ...
                        <span class="basis-[45%] flex justify-center p-1 cursor-pointer gap-2 items-center border-gray-400 border-[1px] rounded-2xl">
                            <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                                stroke="gray" class="w-5 h-5">
                                <path stroke-linecap="round" stroke-linejoin="round"
